...
Deno.test("TypedEventEmitter - Basic functionality", async (t) => {
    ...
    await t.step("should add and trigger string event listeners", () => {
        ...
        const listener: TypedEventListener<typeof testStringEvent> = (
            value,
        ) => {
            receivedValue = value;
        };
...
Deno.test("TypedEventEmitter - Multiple listeners", async (t) => {
    await t.step("should support multiple listeners for same event", () => {
        ...
        emitter.on(
            testStringEvent,
            (value) => results.push(`listener1: ${value}`),
        );
        emitter.on(
            testStringEvent,
            (value) => results.push(`listener2: ${value}`),
        );
        emitter.on(
            testStringEvent,
            (value) => results.push(`listener3: ${value}`),
        );
...
Deno.test("TypedEventEmitter - Removing listeners", async (t) => {
    ...
    await t.step(
        "should handle multiple cleanup functions independently",
        () => {
            const emitter = new TypedEventEmitter();
            let count1 = 0;
            let count2 = 0;
...
Deno.test("TypedEventEmitter - Once listeners", async (t) => {
    ...
    await t.step("should work with multiple once listeners", () => {
        ...
        emitter.once(
            testStringEvent,
            (value) => results.push(`once1: ${value}`),
        );
        emitter.once(
            testStringEvent,
            (value) => results.push(`once2: ${value}`),
        );
...
Deno.test("TypedEventEmitter - Real-world usage patterns", async (t) => {
    await t.step("should support event-driven data flow", () => {
        ...
        const emitter = new TypedEventEmitter();
        let finalResult:
            | (UserData & { type: string; processed: boolean })
            | null = null;
    ...
    await t.step("should support cleanup patterns", () => {
        ...
        const cleanupEvent1 = createTypedEvent<string>(
            "cleanup-pattern-event-1",
        );
        const cleanupEvent2 = createTypedEvent<string>(
            "cleanup-pattern-event-2",
        );
        const cleanupEvent3 = createTypedEvent<string>(
            "cleanup-pattern-event-3",
        );
